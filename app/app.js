...
function bmiInfo(value){
 ...
 if(!Number.isFinite(n)||n<=0)return {label:'Não calculado',tone:'neutral',guide:'Informe peso e altura válidos para calcular o IMC.'};
 if(n<18.5)return {label:'Baixo peso',tone:'attention',guide:'O foco não deve ser perder peso. Vale priorizar alimentação adequada, treino de força e ganho ou preservação de massa muscular, com acompanhamento profissional quando necessário.'};
 if(n<25)return {label:'Peso adequado',tone:'good',guide:'O IMC está na faixa considerada adequada para adultos. O objetivo pode ser manter o peso, melhorar condicionamento e desenvolver massa muscular de forma progressiva.'};
 if(n<30)return {label:'Sobrepeso',tone:'attention',guide:'Uma redução gradual de gordura corporal pode trazer benefícios à saúde. Preserve ou aumente a massa muscular com treino de força e hábitos alimentares adequados; o IMC sozinho não diferencia músculo de gordura.'};
 if(n<35)return {label:'Obesidade grau I',tone:'high',guide:'A redução gradual de peso e gordura corporal tende a ser importante para a saúde, preservando massa muscular com exercícios de força. Considere acompanhamento de profissional de saúde e nutrição.'};
 if(n<40)return {label:'Obesidade grau II',tone:'high',guide:'É recomendável dar prioridade à redução de peso de forma segura e progressiva, preservando massa muscular. Acompanhamento médico e nutricional é especialmente indicado antes de mudanças intensas no treino.'};
 return {label:'Obesidade grau III',tone:'high',guide:'A redução de peso pode trazer benefícios relevantes, mas deve ser planejada com segurança. Procure acompanhamento médico e nutricional e mantenha atividade física compatível com sua condição e orientação profissional.'};
}
function progress(){const bmi=(state.weight/((state.height/100)**2)).toFixed(1),bmiData=bmiInfo(bmi);const last=state.assessments[0],stats=Object.fromEntries(muscleStats());shell(`<div class="progressHead"><div><span class="kicker">MUSY PROGRESS</span><h1>Evolução</h1></div><span class="badge">${state.workouts} treinos</span></div><div class="progressTabs"><button class="on">Mapa muscular</button><button>Medidas</button><button>Histórico</button></div><section class="card muscleCardV9"><div class="anatomyStage">${anatomyFigure(stats)}</div><div class="legend"><span><i></i>Não trabalhado</span><span><i class="worked"></i>Trabalhado</span><span><i class="evolving"></i>Evoluindo</span></div><div class="muscleRows">${muscleStats().map(x=>`<div class="muscleRow"><span class="muscleMini">▦</span><b>${x[0]}</b><div class="bar"><i style="width:${x[1]}%"></i></div><strong>${x[1]}%</strong></div>`).join('')}</div></section><section class="section stats3"><div class="stat"><span>Peso</span><strong>${state.weight} kg</strong></div><div class="stat"><span>IMC</span><strong>${bmi}</strong></div><div class="stat"><span>Cintura</span><strong>${state.waist} cm</strong></div></section><section class="section card bmiCard ${bmiData.tone}"><div class="row bmiTitle"><div><span class="kicker">CLASSIFICAÇÃO DO IMC</span><h2>${bmiData.label}</h2></div><div class="bmiValue">${bmi}</div></div><p>${bmiData.guide}</p>${state.age>=60?'<p class="bmiNote"><b>60+:</b> em pessoas mais velhas, o IMC deve ser interpretado junto com massa muscular, força, alimentação e condição de saúde.</p>':''}<p class="bmiNote">O IMC é uma medida de triagem e não distingue gordura de massa muscular. Ele não substitui avaliação individual de um profissional de saúde.</p></section><section class="section card"><div class="row"><h2>Avaliação corporal</h2><button class="link" id="photo">+ foto</button></div><div class="form2"><label>Peso (kg)<input class="input" id="w" type="number" step="0.1" value="${state.weight}"></label><label>Altura (cm)<input class="input" id="h" type="number" value="${state.height}"></label><label>Cintura (cm)<input class="input" id="wa" type="number" value="${state.waist}"></label><label>Peito (cm)<input class="input" id="ch" type="number" value="${state.chest}"></label><label>Braço (cm)<input class="input" id="ar" type="number" value="${state.arm}"></label><label>Coxa (cm)<input class="input" id="th" type="number" value="${state.thigh}"></label></div><button class="btn" id="eval">SALVAR AVALIAÇÃO</button>${last?`<p class="hint">Última avaliação: ${new Date(last.date).toLocaleDateString('pt-BR')}</p>`:''}</section><section class="section"><h2>Histórico recente</h2>${state.history.slice(0,5).map(h=>`<div class="history"><div><b>${h.title}</b><span>${new Date(h.date).toLocaleDateString('pt-BR')}</span></div><div>${h.completed}/${h.total} • ${h.minutes} min</div></div>`).join('')||'<div class="card muted">Conclua seu primeiro treino para ver o histórico.</div>'}</section><input hidden id="photoInput" type="file" accept="image/*" capture="user">`);$('#eval').onclick=saveAssessment;$('#photo').onclick=()=>$('#photoInput').click();$('#photoInput').onchange=saveProgressPhoto}
